...
def submit_exam_request(request):
    ...
    try:
        ...
        Exception
    ) as e:  # Catch potential errors during creation (like unique constraint violation if check failed)
    ...
    if error_modal_pk and failed_form_data:
        ...
        for exam_data in context["exams_data"]:
            if exam_data["instance"].pk == error_modal_pk:
                ...
                break
        context["error_in_modal_pk"] = (
            error_modal_pk  # Signal template to open this modal
        )
...
def delete_exam(request, pk):
    ...
    month_param = exam.date.strftime(
        "%Y-%m"
    )  # For redirecting back to the correct month
...
def approve_request(request, pk):
    ...
    try:
        ...
        messages.success(
            request,
            f"Solicitud de {request_obj.student.get_full_name()} para '{request_obj.calendar_exam.subject}' aprobada.",
        )
    except Exception as e:
...
def reject_request(request, pk):
    ...
    try:
        ...
        messages.warning(
            request,
            f"Solicitud de {request_obj.student.get_full_name()} para '{request_obj.calendar_exam.subject}' rechazada.",
        )
    except Exception as e:
...
def manage_grades(request):
    ...
    approved_requests = ExamRequest.objects.filter(
        status="Approved", grade__isnull=True
    ).select_related("student", "calendar_exam")
    ...
    for exam_request in approved_requests:
        grading_forms.append(
            {"exam_request": exam_request, "form": GradeForm(instance=exam_request)}
        )
...
def calendar_view(request):
    """
    Decides whether to show the student or teacher calendar view based on the user's role.
    """
    if is_teacher(request.user):
        return redirect("exams:edit_calendar")
    else:
